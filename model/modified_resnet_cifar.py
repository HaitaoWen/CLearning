...
from model import modified_linear
...
class BaseNet(nn.Module):
    ...
    def forward_with_parameters(self, modules, parameters, x):
        for name, module in modules.named_children():
            ...
            # ****************** CosineLinear ******************* #
            elif type(module) is modified_linear.CosineLinear:
                self.pointer += 1
                weight = parameters[self.pointer][1]
                if module.sigma is not None:
                    self.pointer += 1
                    sigma = parameters[self.pointer][1]
                else:
                    sigma = None
                if x.shape[2:4] == (1, 1):
                    x = x.view(x.shape[0], -1)
                input = x.clone()
                x = torch.nn.functional.linear(torch.nn.functional.normalize(x, p=2,dim=1), \
                                                 torch.nn.functional.normalize(weight, p=2, dim=1))
                ...
                if module._forward_hooks:
                    for hook in module._forward_hooks.values():
                        hook(module, (input,), output)
            # ****************** SplitCosineLinear ******************* #
            elif type(module) is modified_linear.SplitCosineLinear:
                if module.sigma is not None:
                    self.pointer += 1
                    sigma = parameters[self.pointer][1]
                else:
                ...
                if x.shape[2:4] == (1, 1):
                    x = x.view(x.shape[0], -1)
                self.pointer += 1
                weight1 = parameters[self.pointer][1]
                # fc1
                input = x.clone()
                x1 = torch.nn.functional.linear(torch.nn.functional.normalize(x, p=2,dim=1), \
                                                torch.nn.functional.normalize(weight1, p=2, dim=1))
                output = x1.clone()
                if module.fc1._forward_hooks:
                    for hook in module.fc1._forward_hooks.values():
                        hook(module.fc1, (input,), output)

                self.pointer += 1
                weight2 = parameters[self.pointer][1]
                # input = x.clone()
                x2 = torch.nn.functional.linear(torch.nn.functional.normalize(x, p=2,dim=1), \
                                                torch.nn.functional.normalize(weight2, p=2, dim=1))
                output = x2.clone()
                if module.fc2._forward_hooks:
                    for hook in module.fc2._forward_hooks.values():
                        hook(module.fc2, (input,), output)
...
if __name__ == "__main__":
    ...
    y__ = model(x)
    model = DerBatchNorm2d.convert_der_batchnorm(model)
